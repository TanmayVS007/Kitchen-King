...
class _GasUsageTrackerScreenState extends State<GasUsageTrackerScreen> {
  ...
  // Scale down the remaining gas for better visualization
  double _getDisplayRemainingGas(double value) {
    // Scale to make it comparable to gas used
    return value / 15; // Adjust the divisor as needed
  }
  ...
  double _getMaxBarValue() {
    ...
    return maxGasUsed > maxRemainingGasScaled ? maxGasUsed : maxRemainingGasScaled;
  }
            ...
            SizedBox(
              ...
              child: LineChart(
                LineChartData(
                  ...
                  titlesData: FlTitlesData(
                    bottomTitles: AxisTitles(
                      sideTitles: SideTitles(
                        showTitles: true,
                        getTitlesWidget: (value, meta) {
                          if (value % 2 == 0 && value >= 0 && value < _gasData.length) {
                            return Padding(
                              padding: const EdgeInsets.only(top: 8.0),
                              child: Text(
                                _formatDate(_gasData[value.toInt()].date),
                  ...
                  minY: showDecreasing
                      ? _gasData.map(getValue).reduce((min, value) => min < value ? min : value) * 0.9
                      : 0,
                  maxY: _gasData.map(getValue).reduce((max, value) => max > value ? max : value) * 1.1,
                  lineBarsData: [
                    LineChartBarData(
